...
def test_setup_containers_no_hosts_shows_empty_state(config_file, data_dir, monkeypatch):
    client = _setup_client(config_file, data_dir, monkeypatch)
    _create_admin()
...
def test_setup_containers_with_containers_renders_rows(config_file, data_dir, monkeypatch):
    client = _setup_client(config_file, data_dir, monkeypatch)
    _create_admin()
    ...
    with patch("app.auth_router.discover_containers", new=AsyncMock(return_value=mock_containers)):
        response = client.get("/setup/containers")
...
def test_setup_containers_host_with_no_containers_shows_empty_state(config_file, data_dir, monkeypatch):
    client = _setup_client(config_file, data_dir, monkeypatch)
    _create_admin()
...
def test_setup_containers_all_checked_by_default(config_file, data_dir, monkeypatch):
    ...
    with patch("app.auth_router.discover_containers", new=AsyncMock(return_value=[
        {"id": "nginx", "name": "nginx", "image": "nginx:latest"}
    ])):
        response = client.get("/setup/containers")
...
def test_setup_containers_save_redirects_to_notifications(config_file, data_dir, monkeypatch):
    client = _setup_client(config_file, data_dir, monkeypatch)
    _create_admin()

    # "Test Host" → slug "test-host" (from conftest default config)
    response = client.post("/setup/containers/save",
...
def test_setup_containers_save_stores_selection(config_file, data_dir, monkeypatch):
    ...
    # "Test Host" → slug "test-host" (from conftest default config)
    client.post("/setup/containers/save",
                data={"containers": ["test-host:nginx", "test-host:plex"]},
                follow_redirects=False)
...
async def test_discover_containers_returns_empty_on_error():
    ...
    with patch("app.ssh_client.asyncssh.connect", side_effect=Exception("connect failed")):
        result = await discover_containers({"name": "test", "host": "1.2.3.4"}, {})
