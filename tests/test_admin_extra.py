...
def test_portainer_test_missing_url_or_key(client):
    response = client.post("/admin/connections/portainer/test", data={
        "portainer_url": "",
        "portainer_api_key": "",
    })
    assert response.status_code == 200
...
def test_portainer_test_connection_ssl_error(client):
    with patch("app.portainer_client.PortainerClient") as MockClient:
        instance = MockClient.return_value
        instance.get_endpoints = AsyncMock(side_effect=Exception("SSL certificate verify failed"))
        response = client.post("/admin/connections/portainer/test", data={
            "portainer_url": "https://portainer.test:9443",
            "portainer_api_key": "test-key",
        })
...
def test_save_portainer_config(client):
    response = client.post("/admin/connections/portainer", data={
...
def test_save_dockerhub_config(client):
    response = client.post("/admin/connections/dockerhub", data={
...
def test_change_password_wrong_current(client):
    response = client.post("/admin/account/password", data={
...
def test_change_password_success(client):
    ...
    assert response.status_code == 200
    assert "saved" in response.text.lower() or "changed" in response.text.lower() or "password" in response.text.lower()
...
def test_regenerate_backup_key_wrong_password(client):
    response = client.post("/admin/account/backup-key", data={
        "current_password": "wrongpassword",
    })
    assert response.status_code == 200
    assert "incorrect" in response.text.lower()
...
def test_post_credentials_error_shows_message(client, monkeypatch):
    ...
    monkeypatch.setattr(a, "save_credentials",
...
def test_docker_monitoring_error_shows_message(client, monkeypatch):
    import app.admin as a
    monkeypatch.setattr(a, "set_docker_monitoring",
...
def test_mfa_remove_wrong_credentials(client):
    ...
    response = client.post("/admin/account/mfa/remove", data={
        "current_password": "wrongpassword",
        "totp_code": "000000",
    })
    assert response.status_code == 200
